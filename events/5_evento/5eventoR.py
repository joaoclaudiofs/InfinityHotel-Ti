...
# Iniciar o PWM nos pinos e definir o objeto da cor verde
green = GPIO.PWM(17, hz)
# Duty cycle - para criar a cor verde -> 100% verde
greendc = 100
...
def controlarLed(status):
    print(f"Status do Som Ambiente: {status}")
    if status == 'Ligado':
        green.start(greendc)  # Liga o LED
    elif status == 'Desligado':
        green.stop()  # Desliga o LED
    else:  # Status diferente de Ligado ou Desligado
...
try:
    while True:
        # Faz um GET à API do estado do som
        status = getFromApi("som")
        if status:  # Se correr bem controla o LED
            controlarLed(status)
    ...
    # Termina programa, desliga a cor verde e limpa os pinos GPIO
    green.stop()
    GPIO.cleanup()
